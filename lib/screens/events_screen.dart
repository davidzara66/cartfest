          ...
          children: [
            Container(
              ...
              child: Column(
                ...
                children: [
                  const Text('PROXIMO EVENTO', style: TextStyle(color: AppTheme.neonCyan, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 8),
                  Text(widget.event.title, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w800)),
                  const SizedBox(height: 6),
                  Text(widget.event.description, style: const TextStyle(color: AppTheme.textSecondary)),
                  ...
                  Row(
                    children: [
                      ...
                      Text(widget.event.location, style: const TextStyle(color: AppTheme.textSecondary)),
                    ],
                  ),
                  const SizedBox(height: 10),
                  SizedBox(
                  ...
                  const Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text('CHAT EN VIVO', style: TextStyle(fontWeight: FontWeight.w700)),
                      ],
                    ),
                  ),
                    ...
                    child: Row(
                      children: [
                        ...
                        IconButton.filled(
                          ...
                          style: IconButton.styleFrom(backgroundColor: AppTheme.neonMagenta),
                        ),
                      ],
                    ),
                  ),
            ...
            child: SingleChildScrollView(
              child: Form(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Registro de Vehiculo al Evento', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
                    const SizedBox(height: 12),
                    _f(vehicleName, 'Nombre del vehiculo'),
                    _f(vehicleBrand, 'Marca'),
                    _f(photoUrl, 'URL foto del vehiculo'),
                    ...
                    DropdownButtonFormField<String>(
                      initialValue: origin,
                      items: kOrigins.map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
                      onChanged: (v) => setModalState(() => origin = v ?? origin),
                      decoration: const InputDecoration(labelText: 'Clasificado por origen'),
                    ),
                    const SizedBox(height: 8),
                    DropdownButtonFormField<String>(
                      initialValue: classification,
                      items: kClassifications.map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
                      onChanged: (v) => setModalState(() => classification = v ?? classification),
                      decoration: const InputDecoration(labelText: 'Clasificacion de suscripcion'),
                    ),
                    const SizedBox(height: 10),
                    const Text('Secciones a participar'),
                    Wrap(
                      spacing: 8,
                      children: ['Motor', 'Suspension', 'Estetica', 'Audio', 'Aero']
                          .map((s) => FilterChip(
                                label: Text(s),
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          ...
                          provider.registerToEvent(
                            ...
                            vehiclePhotoUrl: photoUrl.text.trim(),
                            category: category,
                            origin: origin,
                          ...
                          Navigator.pop(ctx);
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Registro enviado correctamente')));
                        },
  ...
  Widget _countBox(int value, String label) {
    return Expanded(
      child: Container(
        ...
        child: Column(
          children: [
            Text(value.toString().padLeft(2, '0'), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800)),
